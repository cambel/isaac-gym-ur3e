"""
Universal Robots UR3e Cube Pick with FT sensor
----------------
Use Jacobian matrix and inverse kinematics control of ur3e robot to pick up a box.
...
body_idx = gym.find_asset_rigid_body_index(ur3e_asset, "tool0")
...
sensor_idx = gym.create_asset_force_sensor(ur3e_asset, body_idx, sensor_pose, sensor_props)
...
if controller == "ik":
    ...
    ur3e_dof_props["damping"][:6].fill(100.0)
else:       # osc
...
# grippers open
default_dof_pos[6:] = ur3e_lower_limits[6:]
...
ur3e_pose.p = gymapi.Vec3(0, 0, 0.4)
...
for i in range(num_envs):
    ...
    actors.append(ur3e_handle)
    ft_sensors.append(gym.get_actor_force_sensor(env, ur3e_handle, sensor_idx))
...
# get jacobian tensor
# for fixed-base ur3e, tensor has shape (num envs, 10, 6, 9)
_jacobian = gym.acquire_jacobian_tensor(sim, "ur3e")
jacobian = gymtorch.wrap_tensor(_jacobian)

# jacobian entries corresponding to ur3e hand
j_eef = jacobian[:, ur3e_hand_index - 1, :, :6]

# get mass matrix tensor
_massmatrix = gym.acquire_mass_matrix_tensor(sim, "ur3e")
mm = gymtorch.wrap_tensor(_massmatrix)
mm = mm[:, :6, :6]          # only need elements corresponding to the ur3e arm
...
while not gym.query_viewer_has_closed(viewer):
    ...
    # print("gripped", bool(gripped))
    # print("gripped", bool(gripped), float(gripper_sep), float(box_dist), grasp_offset + 0.5 * box_size)
    # if gripped:
    #     print("?????????????????????????")
    ...
    # print("return to start?", bool(return_to_start))
    ...
    # print("** hand restart?", bool(hand_restart))
    # print("close?", bool(close_gripper))
    # print("gripper?", grip_acts)
    pos_action[:, 6:8] = grip_acts

    print("FT sensor", fsdata)
    # print("FT sensor", gym.get_actor_force_sensor(envs[0], actors[0], sensor_idx).get_forces().force)
